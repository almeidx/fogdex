...
const filterParsers = {
	...
	lullaby: parseAsBoolean.withDefault(false),
	...
	return killers.filter((k) => {
		if (filters.search) {
			...
			const matches =
				...
				k.gameTags.some((tag) => tag.toLowerCase().includes(q)) ||
				k.powerName.toLowerCase().includes(q) ||
				k.weapon.toLowerCase().includes(q);
			if (!matches) return false;
		}

		if (filters.speedMin !== null && k.speed < filters.speedMin) return false;
		if (filters.speedMax !== null && k.speed > filters.speedMax) return false;
		...
		if (filters.hasLullaby && k.lullabyRadius === null) return false;
