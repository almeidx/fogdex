...
export function SurvivorPerksPage() {
	const [perks, setPerks] = useState<Perk[]>([]);
	const [loading, setLoading] = useState(true);

	useEffect(() => {
		import("../data/survivor-perks.json").then((m) => {
	...
	if (loading) {
		return (
			<main className="flex-1 flex items-center justify-center px-4">
				<p className="text-text-muted">Loading survivor perks...</p>
			</main>
		);
	}
		...
		return (
			<main className="flex-1 flex items-center justify-center px-4">
				<div className="text-center text-text-muted">
					<p className="text-lg">No survivor perk data loaded yet</p>
