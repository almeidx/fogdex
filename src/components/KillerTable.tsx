...
function SortHeader({
	...
	return (
		<th
			...
			className="sticky bg-surface/95 backdrop-blur-sm py-3 px-2"
...
export function KillerTable({ killers, sort, onSortChange, onClearFilters }: KillerTableProps) {
	...
	return (
		<>
			<span className="sr-only" role="status">
				Sorted by {SORT_LABELS[sort.column]}, {sort.direction === "asc" ? "ascending" : "descending"}
			</span>
			<div className="hidden md:block">
				<table className="w-full">
					<caption className="sr-only">Dead by Daylight killers</caption>
					<thead>
					...
					<tbody>
						{killers.map((killer, i) => (
							<KillerRow key={killer.slug} killer={killer} priority={i < 10} />
						))}
					</tbody>
				</table>
			</div>
			...
			<div className="space-y-3 md:hidden">
				{killers.map((killer, i) => (
					<KillerCard key={killer.slug} killer={killer} priority={i < 4} />
				))}
			</div>
		</>
	);
