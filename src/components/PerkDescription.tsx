...
function segmentDescription(text: string, tierPatterns: Set<string>, statusEffects: StatusEffects): Segment[] {
	...
	const statusNames = Object.keys(statusEffects);
		...
		<p className="text-sm leading-relaxed text-text/80">
			{segments.map((seg) => {
				switch (seg.type) {
					case "tier":
						return (
							<span className="font-semibold text-accent-text" key={seg.offset}>
								{seg.value}
							</span>
						);
					case "status":
						return <StatusEffectTooltip description={statusEffects[seg.value]} key={seg.offset} name={seg.value} />;
					default:
						return seg.value;
				}
			})}
