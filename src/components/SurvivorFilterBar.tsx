	...
	return (
		<div className="sticky top-0 z-10 border-b border-border bg-surface/95 backdrop-blur-sm" ref={ref}>
			<span className="sr-only" role="status">
				{mounted ? `${filteredCount} of ${totalCount} survivors` : ""}
			</span>
			<div className="md:hidden flex items-center justify-between px-4 py-3">
				<button
					aria-controls="survivor-filter-panel"
			...
			<div className={`${mobileOpen ? "block" : "hidden"} md:block px-4 py-3`} id="survivor-filter-panel">
				<div className="mx-auto max-w-350 flex flex-col gap-3 md:flex-row md:flex-wrap md:items-center md:gap-2">
					...
					<div className="flex items-center gap-1">
						{(["all", "yes", "no"] as const).map((v) => (
							<button
								aria-label={
									v === "all" ? "All survivors" : v === "yes" ? "Licensed survivors only" : "Original survivors only"
								}
